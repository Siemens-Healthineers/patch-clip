...
class CXRTestDataset(data.Dataset):
    ...
    def __init__(
        ...
        self.samples = [l[:-1] for l in open(label_list, 'r').readlines()]        
    ...
    def __getitem__(self, idx):
        ...
        splitted = self.samples[idx]
        sfname = splitted.strip().split(',') #',' for test file 
        label = int(sfname[1])
        plbl = []   
        ...
        if True:
                if (len(plbl)<self.label_limit): #this is needed to make all the batch plbls the same length, set to max
                    ...
                    print(sfname[0], plbl," print the patch labels for image")
                    print("Length of the label",len(plbl)) 

        if self.transform:
            img = self.transform(img)
            
        sample = {'img': img, 'lbl': label, 'plbl': plbl}
...
def load_clip(model_path, pretrained=False, context_length=77): 
    ...
    if pretrained is False: 
        ...
        model = CLIP(**params)
        #print("Load new CLIP model with specified params")
    else: 
        model, preprocess = clip.load("ViT-B/32", device=device, jit=False) 
    try: 
        print("Loading this model - ", model_path,)
...
def predict(loader, model, zeroshot_weights, softmax_eval=True, verbose=0): 
    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm(loader)):
            ...
            # predict
            print(images.shape,"Images shape in predict, zero_shot.py line 164")
            image_features = model.encode_image(images) 
            image_features /= image_features.norm(dim=-1, keepdim=True) # (1, 768)

            # obtain logits
            #print("Size of the zeroshot weights", zeroshot_weights.size())
            logits = image_features @ zeroshot_weights # (1, num_classes)
            #print("Print the logits inside zeroshot_classifier",logits)
...
def make(
    ...
    # load data
    #commenting out as the local hd5 file of siemens data is already normalized before saving
    #transformations = [
...
def ensemble_models(
    ...
    for path in model_paths: # for each model
        ...
        if cache_dir is not None and os.path.exists(cache_path): 
            ...
            y_pred = run_softmax_eval(model, loader, cxr_labels, cxr_pair_template, context_length=context_length)
            if cache_dir is not None: 
